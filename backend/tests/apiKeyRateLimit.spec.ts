...
describe("API Key & Rate Limiting", () => {
  ...
  it("should regenerate API Key", async () => {
    const { user, accessToken } = await createTestUser("regen@example.com", "Regen User");
  ...
  it("should get API key via GET endpoint", async () => {
    const { user, accessToken } = await createTestUser("getkey@example.com", "GetKey User");
  ...
  it("should allow access with Bearer token", async () => {
    const { accessToken } = await createTestUser("bearer@example.com", "Bearer User");
  ...
  it("should use API key to create short URL", async () => {
    const { user } = await createTestUser("urlcreate@example.com", "URL Creator");
    await resetRateLimit(user._id.toString());
  ...
  it("should return rate limit error message", async () => {
    const { user } = await createTestUser("ratelimitmsg@example.com", "Rate Limit Msg User");
    await resetRateLimit(user._id.toString());
    for (let i = 0; i < 10; i++) {
      await request(app)
        .get("/api/v1/user/current-user")
  ...
  it("should not rate limit Bearer token requests", async () => {
    const { accessToken } = await createTestUser("norlbearer@example.com", "No RL Bearer");
    for (let i = 0; i < 15; i++) {
      const res = await request(app)
        .get("/api/v1/user/current-user")
        .set("Authorization", `Bearer ${accessToken}`);
  ...
  it("should reject deleted user API key", async () => {
    const { user } = await createTestUser("deleted@example.com", "Deleted User");
    await resetRateLimit(user._id.toString());
