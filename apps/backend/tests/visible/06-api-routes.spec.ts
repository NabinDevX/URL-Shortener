...
const extractData = (body: ApiResponse): Record<string, any> => {
  return ((body.result?.data as Record<string, any>) || body) as Record<
    string,
    any
  >;
};
...
describe("API Routes Integration", () => {
  ...
  it("should create short URL and return existing one for duplicate redirect URL", async () => {
    ...
    const redirectUrl = "https://example.com/dup";
  ...
  it("should update URL QR code, list user URLs, and toggle soft delete", async () => {
    ...
    const listRes = await request(app)
      .get(
        "/api/v1/url/user/all?page=1&limit=5&sortBy=createdAt&sortOrder=desc"
      )
      .set("Authorization", `Bearer ${accessToken}`);
