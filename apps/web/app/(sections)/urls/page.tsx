...
export default function UrlsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6 lg:p-8 intro-reveal">
      {message.text && (
        <div
          className={`fixed bottom-6 right-6 z-50 animate-fade-in rounded-xl px-4 py-3 text-sm font-semibold shadow-xl border ${message.type === "success" ? "bg-emerald-50 text-emerald-800 border-emerald-200" : "bg-red-50 text-red-800 border-red-200"}`}
        >
          <span>
                    ...
                    <div className="flex gap-1.5 mt-2">
                      {[4, 6, 8, 10, 12].map((len) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-colors ${parseInt(String(newUrl.idLength)) === len ? "bg-primary text-white" : "bg-surface-container-high text-on-surface-variant hover:bg-surface-container-highest"}`}
                        >
                          {len}
                        </button>
                      ))}
