...
const URLS = () => {
  ...
  const handleCreateUrl = async (
    ...
    try {
      ...
      if (axiosError instanceof AxiosError) {
        ...
        if (status === 409) {
          const hasCustom = Boolean(newUrl.customShortId?.trim());
          errorMessage = hasCustom
            ? "That custom short ID is already taken. Please try another one."
            : backendMessage ||
              "This URL already exists. Try a different URL or use a custom short ID.";
  ...
  const copyToClipboard = (shortId: string): void => {
    const fullUrl = `https://${window.location.host}/s/${shortId}`;
            ...
            <form onSubmit={handleCreateUrl} className="space-y-6">
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
                ...
                <ul className="text-xs text-blue-700 mt-2 ml-6 space-y-1">
                  <li>
                    • Create a <strong>custom short ID</strong> (e.g., my-link)
                  </li>
                  <li>
                  ...
                  {newUrl.customShortId && (
                    <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                      <p className="text-xs text-green-700 font-semibold">
                        Preview: https://{window.location.host}/s/
          ...
          {urls.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {urls.map((url) => {
                ...
                const fullShortUrl = `https://${window.location.host}/s/${url.shortId}`;
