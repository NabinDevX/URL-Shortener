...
const Signup = () => {
  ...
  const { login: authLogin, signupWithGoogleCode } = useAuth();
  ...
  const handleGoogleSignupCode = async (code: string) => {
    if (!code) {
      setErrors({ general: "Google sign-up failed. Missing code." });
      return;
    }
    ...
    try {
      await signupWithGoogleCode(code);
  ...
  const handleOtpPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < pastedData.length && i < 6; i++) {
      newOtp[i] = pastedData[i] as string;
    }
    setOtp(newOtp);
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (axiosError instanceof AxiosError) {
        const errorMessage =
          axiosError.response?.data?.message || "Invalid OTP";
              ...
              <>
                {googleClientId ? (
                  <div className="mb-6">
                    ...
                    >
                      <GoogleCodeSignupButton
                        disabled={googleLoading}
                        onCode={handleGoogleSignupCode}
                        onError={() => {
