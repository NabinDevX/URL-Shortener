...
export default function Dashboard() {
  ...
  const fetchUrls = useCallback(async () => {
    ...
    try {
      const response = await requestOnce(
        "url:user:all",
        () =>
          axios.get("/url/user/all", { withCredentials: true, timeout: 10000 }),
        2500
      );
  ...
  const countClicksInRange = (startMs: number, endMs: number): number => {
    return urls.reduce((sum: number, url: any) => {
      const visits = Array.isArray(url.visitHistory) ? url.visitHistory : [];
      const count = visits.filter((visit: any) => {
        const ts = visit?.timestamp ? new Date(visit.timestamp).getTime() : 0;
  ...
  const last7Clicks = countClicksInRange(last7Start, now);
  const prev7Clicks = countClicksInRange(prev7Start, last7Start);
  const weekChangePct =
    prev7Clicks > 0
      ? Math.round(((last7Clicks - prev7Clicks) / prev7Clicks) * 100)
      : null;
    ...
    <div className="mx-auto max-w-7xl space-y-10 p-4 sm:space-y-12 sm:p-6 lg:p-8 intro-reveal">
      <div className="space-y-2">
        ...
        <p className="text-on-surface-variant max-w-xl leading-relaxed">
          View your link performance for the last 7 days. Upgrade to Pro for
          detailed demographics and lifetime history.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-2 bg-primary-container p-6 rounded-3xl relative overflow-hidden group hover-float">
          <div className="relative z-10">
            <p className="text-sm font-semibold text-primary-fixed mb-4">
              Pro Feature Insight
            </p>
            <h3 className="text-xl font-bold text-white mb-2 font-headline">
              Geographic Breakdown
            </h3>
            <p className="text-primary-fixed/80 text-sm max-w-60">
              See exactly where your traffic is coming from down to the city
              level.
            </p>
            <button className="mt-4 px-4 py-2 bg-white text-primary text-xs font-bold rounded-xl hover:scale-105 active:scale-95 transition-transform">
              Unlock Analytics
            </button>
          </div>
            ...
            <div className="flex justify-between items-center mb-10">
              <div>
                ...
                <p className="text-sm text-on-surface-variant">
                  Click distribution by destination
                </p>
              </div>
              <div className="flex gap-2">
                <button className="px-3 py-1.5 bg-white text-xs font-bold text-on-surface rounded-lg shadow-sm border border-outline-variant/10">
                  7 Days
                </button>
                <button className="px-3 py-1.5 text-xs font-bold text-on-surface-variant rounded-lg hover:bg-white/50 transition-colors opacity-50 cursor-not-allowed">
                  30 Days{" "}
            ...
            <div className="space-y-8">
              {top5.length > 0 ? (
                  ...
                  return (
                    <div key={url.shortId} className="space-y-2">
                      ...
                      <div className="h-3 w-full bg-white rounded-full overflow-hidden">
                        <div
                          className="h-full pro-gradient rounded-full transition-all duration-700"
                ...
                <p className="text-sm text-on-surface-variant text-center py-8">
                  No URLs yet. Create a link to see data here.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-surface-container-lowest p-8 rounded-4xl border border-outline-variant/10 shadow-sm flex flex-col h-full">
            ...
            <div className="mt-8 pt-8 border-t border-surface-container space-y-4">
              ...
              <p className="text-sm text-on-surface-variant leading-relaxed">
                Free users only see total click volume. Pro users unlock
                referral sources, browser types, and OS data.
              </p>
              <button className="w-full py-3 bg-tertiary-fixed text-on-tertiary-fixed font-bold rounded-2xl hover:bg-tertiary-fixed-dim transition-colors">
                Compare Plans
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-on-surface-variant text-sm">
              Managing {activeUrls} of 50 total free links
            </p>
          </div>
          <Link
            href="/urls"
            className="flex w-full items-center justify-center gap-2 rounded-2xl px-6 py-3 font-bold text-white shadow-lg shadow-primary/20 transition-transform hover:scale-105 active:scale-95 sm:w-auto pro-gradient"
          >
              ...
              <tbody className="divide-y divide-surface-container">
                {urls.slice(0, 5).map((url) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-5 text-sm text-on-surface-variant truncate max-w-xs">
                      {url.redirectUrl}
                    </td>
                    ...
                    <td className="px-8 py-5 text-right">
                      ...
                      <Link
                        href="/urls"
                        className="p-2 text-slate-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-all inline-block"
                      >
                        <span className="material-symbols-outlined text-xl">
                          edit
                        </span>
      ...
      <div className="pro-gradient p-12 rounded-[2.5rem] relative overflow-hidden shadow-2xl shadow-primary/30">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-left space-y-4">
            ...
            <p className="text-primary-fixed max-w-md font-medium">
              Get custom domains, bulk shortening, and detailed UTM tracking for
              your entire marketing team.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              <button className="px-8 py-4 bg-white text-primary font-extrabold rounded-2xl shadow-xl hover:-translate-y-1 transition-transform">
                Start Pro Free Trial
              </button>
              <button className="px-8 py-4 bg-primary-container text-white font-extrabold rounded-2xl border border-white/20 hover:bg-primary/80 transition-colors">
                Book a Demo
              </button>
            </div>
          </div>
