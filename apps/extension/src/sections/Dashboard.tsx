...
const Dashboard = ({ userData }: DashboardProps) => {
  ...
  const [, setAnalytics] = useState<AnalyticsMap>({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedUrl, setSelectedUrl] = useState<string | null>(null);
  const [fetchDone, setFetchDone] = useState(false);

  const fetchAllAnalytics = useCallback(async (urlsList: UrlItem[]) => {
    try {
      ...
      urlsList.forEach((url, index: number) => {
        const analyticsResult = analyticsResults.at(index);
        const analyticsData = analyticsResult?.data?.data;
        analyticsMap[url.shortId] = analyticsData ?? { totalClicks: 0 };
      });
  ...
  const fetchUrls = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        (error.response?.data?.data as { urls?: UrlItem[] })?.urls?.length === 0
      ) {
        setUrls([]);
      } else {
        setError(error.response?.data?.message || "Failed to load URLs");
      }
    } finally {
      setLoading(false);
    }
